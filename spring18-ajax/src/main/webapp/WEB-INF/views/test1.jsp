...
<script>
	$(function(){
		//아이디 입력창의 input/blur 시점에 비동기통신을 이용하여 화면을 유지한 채로 아이디 중복 검사 수행
		$("input[name=memberId]").blur(function(){
			...
			$.ajax({ //this : ajax객체
				...
				type:"post",
				success:function(resp){ //"Y" 사용가능, "N" 사용중으로 처리
					if(resp === "Y"){
						$("input[name=memberId] + span").text("사용 가능한 아이디 입니다");
					}
					else if(resp === "N"){
						$("input[name=memberId] + span").text("이미 사용중인 아이디 입니다");
					}
				}
			});
